...
| `docker compose down` hangs: `container ... is zombie and can not be killed` | Container's process spawned subprocesses (Streamlit's live-mode rerun loop, in particular) that weren't being reaped by PID 1 | Already fixed via `init: true` on all three services in `docker-compose.yml`. If it ever recurs: `docker kill <name>` then `docker rm -f <name>` to clear the stuck container; data in named volumes is untouched either way |
